# RCZ_v6_PREDICTIVE.py
# Full predictive upgrade of RCZ_v5_FULLY_ADAPTIVE.py
# Objective: TRUE forward-outcome prediction, multi-horizon forecasting,
# price location intelligence, timing separation, regime transition prediction,
# adverse-path modeling, model health monitoring, and meta-decision layer.
...
PREDICTION_BUFFER_FILE = "apex_prediction_buffer_v9.json"
...
LOGISTIC_DIM       = 12  # Increased for additional predictive features
...
# Predictive horizons in seconds
PREDICTION_HORIZONS = [5, 10, 20, 30, 60]
...
#============================================================================
# PREDICTION BUFFER (for delayed learning without lookahead)
#============================================================================

class PredictionBuffer:
    """
    Stores feature snapshots at signal time for later outcome labeling.
    Prevents lookahead bias by separating prediction time from outcome time.
    """
    def __init__(self, path: str):
        self.path = path
        self.buffer = []  # List of pending predictions awaiting outcomes
        ...
        try:
            if os.path.exists(self.path):
                with open(self.path, "r") as f:
                    data = json.load(f)
                self.buffer = data.get("buffer", [])[-self.max_buffer_size:]
        except Exception as e:
            log("WARN", f"Prediction buffer load failed: {e}")
            self.buffer = []

    def save(self):
        try:
            with open(self.path, "w") as f:
                json.dump({"buffer": self.buffer[-self.max_buffer_size:]}, f, indent=2, default=json_default)
        except Exception as e:
            log("WARN", f"Prediction buffer save failed: {e}")

    def add_snapshot(self, ts: float, features: dict, prediction: dict, context: dict):
        """Store prediction state for later labeling."""
        self.buffer.append({
            "ts": ts,
            "features": features,
    ...
    def get_ready_for_labeling(self, current_time: float):
        """
        Return snapshots where all horizons have elapsed.
        Max horizon is 60 seconds, so we wait 65s to be safe.
        """
        ready = []
        remaining = []
        max_horizon = max(PREDICTION_HORIZONS) + 5  # Add 5s safety margin

        for item in self.buffer:
            if current_time - item["ts"] >= max_horizon:
                ready.append(item)
            else:
    ...
    def clear_old(self, current_time: float, max_age_seconds: float = 300.0):
        """Remove very old snapshots that will never be labeled."""
        self.buffer = [item for item in self.buffer if current_time - item["ts"] < max_age_seconds]
        self.save()

#============================================================================
# FORWARD OUTCOME PREDICTOR (TRUE MULTI-HORIZON PREDICTION)
#============================================================================

class ForwardOutcomePredictor:
    """
    TRUE forward-outcome prediction engine.
    Learns relationships between CURRENT market state and FUTURE returns.
    Uses multiple horizons: +5s, +10s, +20s, +30s, +60s.
    
    At signal time: ONLY uses information available NOW.
    After horizon elapses: Labels outcome and updates model.
    """
    def __init__(self):
        # Separate model weights for each horizon
        self.horizon_models = {}
        for h in PREDICTION_HORIZONS:
            # Features: [bias, vol_rank, trend_score, momentum_state, micro_bias, 
            #           location_score, agreement, conflict, spread_rank, regime_code]
            self.horizon_models[h] = np.zeros(10)
        
        self.calibration_stats = {h: {"total": 0, "wins": 0, "sum_error": 0.0} for h in PREDICTION_HORIZONS}

    def _feature_vector(self, state: dict, comps: dict, location: dict, timing: dict) -> np.ndarray:
        """
        Build feature vector from CURRENT state only.
        NO future information allowed.
        """
        vol_rank = clamp(state.get("vol_rank", 0.5), 0.0, 1.0)
        trend_score = clamp(state.get("trend_score", 0.5), 0.0, 1.0)
        momentum_state = clamp(state.get("momentum_state", 0.0), -0.5, 0.5) * 2.0  # Scale to [-1, 1]
        spread_rank = clamp(state.get("spread_rank", 0.5), 0.0, 1.0)
        ...
        agreement = clamp(state.get("agreement", 0.5), 0.0, 1.0)
        conflict = clamp(state.get("conflict", 0.0), 0.0, 1.0)
        
        # Price location quality
        location_score = location.get("location_score", 0.5)
        
        # Timing quality
        timing_quality = timing.get("timing_quality", 0.5)
        
        # Microstructure bias
        micro_bias = state.get("micro_bias", 0.0)
    ...
    def predict_horizons(self, state: dict, comps: dict, location: dict, timing: dict) -> dict:
        """
        Generate predictions for ALL horizons using current state.
        Returns probability of favorable move and expected return for each horizon.
        """
        features = self._feature_vector(state, comps, location, timing)
        ...
        for h in PREDICTION_HORIZONS:
            weights = self.horizon_models[h]
            z = float(np.dot(weights, features))
            prob_favorable = sigmoid(z)  # Probability of favorable directional move
            
            # Expected return approximation (scaled by horizon)
            horizon_scale = h / 30.0  # Normalize around 30s
            expected_return = (prob_favorable - 0.5) * 0.8 * horizon_scale
            
            results[h] = {
                "probability": prob_favorable,
                "expected_return": expected_return,
            }
            
            if prob_favorable > 0.52:
                buy_signals += 1
                total_prob_buy += prob_favorable
            elif prob_favorable < 0.48:
                sell_signals += 1
                total_prob_sell += (1.0 - prob_favorable)
        
        # Aggregate metrics
        n_horizons = len(PREDICTION_HORIZONS)
        avg_prob = np.mean([results[h]["probability"] for h in PREDICTION_HORIZONS])
        avg_return = np.mean([results[h]["expected_return"] for h in PREDICTION_HORIZONS])
        
        # Check alignment across horizons
        aligned_bullish = buy_signals >= 4  # Majority of horizons bullish
        aligned_bearish = sell_signals >= 4  # Majority of horizons bearish
        
        # Trend in probabilities (shorter vs longer horizon)
        short_avg = np.mean([results[h]["probability"] for h in [5, 10]])
        long_avg = np.mean([results[h]["probability"] for h in [30, 60]])
        trend = short_avg - long_avg  # Positive = strengthening, negative = weakening
        
        return {
            "horizon_predictions": results,
    ...
    def update_model(self, horizon: int, features: np.ndarray, actual_return: float,
                     predicted_direction: int):
        """
        Update horizon-specific model with actual outcome.
        Uses online learning with recency weighting.
        """
        if horizon not in self.horizon_models:
            return

        # Determine label: positive return = win for predicted direction
        if predicted_direction > 0:
            y = 1.0 if actual_return > 0 else 0.0
        elif predicted_direction < 0:
            y = 1.0 if actual_return < 0 else 0.0
        else:
            y = 0.5

        weights = self.horizon_models[horizon]
        z = float(np.dot(weights, features))
        p = sigmoid(z)

        # Gradient descent update
        lr = 0.008 * recency_weight(time.time())  # Recency-weighted learning rate
        grad = (p - y) * features + 0.0001 * weights  # L2 regularization
        weights -= lr * grad
        weights = np.clip(weights, -2.5, 2.5)

        self.horizon_models[horizon] = weights

        # Update calibration stats
        pred_win = (p > 0.5 and y == 1.0) or (p < 0.5 and y == 0.0)
        self.calibration_stats[horizon]["wins"] += 1 if pred_win else 0
        self.calibration_stats[horizon]["total"] += 1
        self.calibration_stats[horizon]["sum_error"] += abs(p - y)

    def get_model_health(self, horizon: int) -> dict:
        """Get health metrics for a specific horizon model."""
        stats = self.calibration_stats.get(horizon, {"total": 0, "wins": 0, "sum_error": 0.0})
        ...
        accuracy = stats.get("wins", 0) / total
        mean_error = stats.get("sum_error", 0.5) / total

        if accuracy < 0.45 or mean_error > 0.35:
            health = "DEGRADED"
        elif accuracy < 0.50 or mean_error > 0.25:
            health = "SUBOPTIMAL"
        else:
            health = "HEALTHY"
    ...
    def get_overall_health(self) -> dict:
        """Aggregate health across all horizons."""
        health_counts = {"HEALTHY": 0, "SUBOPTIMAL": 0, "DEGRADED": 0, "INSUFFICIENT_DATA": 0}
        total_samples = 0
        avg_accuracy = 0.0

        for h in PREDICTION_HORIZONS:
            h_health = self.get_model_health(h)
            health_counts[h_health["health"]] += 1
            total_samples += h_health["sample_count"]
            avg_accuracy += h_health["accuracy"]

        avg_accuracy /= len(PREDICTION_HORIZONS)

        if health_counts["DEGRADED"] >= 3:
            overall = "CRITICAL"
        elif health_counts["DEGRADED"] >= 1 or health_counts["INSUFFICIENT_DATA"] >= 4:
            overall = "WARNING"
        elif health_counts["SUBOPTIMAL"] >= 2:
            overall = "CAUTION"
        else:
            overall = "OPERATIONAL"
...
class PriceLocationEngine:
    """
    Determines where current price sits relative to key levels.
    Critical for entry timing quality assessment.
    """
    def __init__(self):
        pass

    def analyze(self, df, state: dict) -> dict:
        """
        Analyze current price location relative to important levels.
        Returns location score and detailed positioning info.
        """
        ...
        # Session levels
        session_high = float(df['high'].iloc[-max(1, int(60)):].max())  # ~1 hour
        session_low = float(df['low'].iloc[-max(1, int(60)):].min())
        session_range = session_high - session_low
        
        # Recent swing levels
        highs_20 = df['high'].values[-21:-1] if len(df) > 21 else np.array([close])
        lows_20 = df['low'].values[-21:-1] if len(df) > 21 else np.array([close])
        swing_high = float(np.max(highs_20)) if len(highs_20) > 0 else close
        swing_low = float(np.min(lows_20)) if len(lows_20) > 0 else close
        
        # EMA positions
        ema9 = safe_float(df['ema_9'].iloc[-1], close)
        ema21 = safe_float(df['ema_21'].iloc[-1], close)
        ema50 = safe_float(df['ema_50'].iloc[-1], close)
        
        # Range midpoint
        range_mid = (session_high + session_low) / 2.0
        
        # Calculate position metrics
        dist_to_session_high = (session_high - close) / max(atr, 0.01)
        dist_to_session_low = (close - session_low) / max(atr, 0.01)
        dist_to_swing_high = (swing_high - close) / max(atr, 0.01)
        dist_to_swing_low = (close - swing_low) / max(atr, 0.01)
        
        # Position within session range (0 = low, 1 = high)
        if session_range > 0:
            pct_in_range = (close - session_low) / session_range
        else:
            pct_in_range = 0.5
        
        # Distance from EMAs
        dist_from_ema9 = abs(close - ema9) / max(atr, 0.01)
        dist_from_ema21 = abs(close - ema21) / max(atr, 0.01)
        
        # Location scoring (higher = better for entries)
        # Best locations: near support in uptrend, near resistance in downtrend
        location_score = 0.5
        
        # Bonus for being near range extremes (potential reversal points)
        if dist_to_session_high < 0.3 or dist_to_session_low < 0.3:
            location_score += 0.15
        elif dist_to_session_high < 0.6 or dist_to_session_low < 0.6:
            location_score += 0.08
        
        # Bonus for being near swing levels
        if dist_to_swing_high < 0.4 or dist_to_swing_low < 0.4:
            location_score += 0.12
        
        # Penalty for being in middle of nowhere
        if 0.35 < pct_in_range < 0.65 and dist_from_ema9 > 0.8:
            location_score -= 0.15
        
        # Penalty for extreme extension
        if dist_from_ema9 > 2.0:
            location_score -= 0.20
        elif dist_from_ema9 > 1.5:
            location_score -= 0.10
        
        location_score = clamp(location_score, 0.0, 1.0)
        
        # Determine qualitative position
        if pct_in_range > 0.85:
            position = "NEAR_SESSION_HIGH"
        elif pct_in_range > 0.65:
            position = "UPPER_RANGE"
        elif pct_in_range > 0.35:
            position = "MID_RANGE"
        elif pct_in_range > 0.15:
            position = "LOWER_RANGE"
        else:
            position = "NEAR_SESSION_LOW"
...
class TimingIntelligence:
    """
    Separates DIRECTION from TIMING.
    Evaluates whether NOW is the right moment to enter.
    """
    def __init__(self):
        pass

    def evaluate(self, df, state: dict, comps: dict, location: dict) -> dict:
        """
        Assess entry timing quality independent of direction.
        """
        ...
        close = float(df['close'].iloc[-1])
        ema9 = safe_float(df['ema_9'].iloc[-1], close)
        dist_from_ema = abs(close - ema9) / max(atr, 0.01)
        
        if dist_from_ema > 2.0:
            timing_score -= 0.25
            reasons.append("Overextended from EMA")
        elif dist_from_ema > 1.5:
            timing_score -= 0.15
            reasons.append("Extended from EMA")
        elif dist_from_ema < 0.5:
            timing_score += 0.10
            reasons.append("Near EMA zone")
        
        # 2. Pullback quality
        pb_comp = comps.get("pullback", {})
        if pb_comp.get("direction", 0) != 0 and pb_comp.get("strength", 0.0) > 0.3:
            timing_score += 0.15
            reasons.append("Quality pullback detected")
        
        # 3. Candle sequence
        recent_candles = df.iloc[-5:]
        body_ratios = recent_candles['body_ratio'].values
        avg_body = np.mean(body_ratios)
        
        if avg_body > 0.7:
            timing_score -= 0.10
            reasons.append("Large candles (potential exhaustion)")
        elif avg_body < 0.3:
            timing_score += 0.08
            reasons.append("Consolidation (coiling)")
        
        # 4. Momentum maturity
        rsi_now = safe_float(df['rsi'].iloc[-1], 50.0)
        if rsi_now > 75 or rsi_now < 25:
            timing_score -= 0.15
            reasons.append("Momentum extreme (late entry risk)")
        elif 45 <= rsi_now <= 55:
            timing_score += 0.08
            reasons.append("Momentum neutral (room to run)")
        
        # 5. Breakout freshness
        breakout_prob = state.get("breakout_prob", 0.5)
        if breakout_prob > 0.7:
            # Check if this is fresh or mature
            highs_recent = df['high'].values[-10:]
            if close > np.max(highs_recent[:-1]):
                timing_score += 0.12
                reasons.append("Fresh breakout")
            else:
                timing_score -= 0.10
                reasons.append("Mature breakout (late)")
        
        # 6. Location confirmation
        loc_score = location.get("location_score", 0.5)
        if loc_score > 0.65:
            timing_score += 0.12
            reasons.append("Good location")
        elif loc_score < 0.35:
            timing_score -= 0.15
            reasons.append("Poor location")
        
        # 7. Microstructure confirmation
        micro_bias = state.get("micro_bias", 0.0)
        if abs(micro_bias) > 0.2:
            timing_score += 0.10
            reasons.append("Microstructure confirmed")
        
        timing_score = clamp(timing_score, 0.0, 1.0)
        
        # Qualitative state
        if timing_score >= 0.70:
            timing_state = "EXCELLENT"
        elif timing_score >= 0.55:
            timing_state = "GOOD"
        elif timing_score >= 0.40:
            timing_state = "ACCEPTABLE"
        elif timing_score >= 0.25:
            timing_state = "POOR"
        else:
            timing_state = "VERY_POOR"
        
        return {
            "timing_quality": timing_score,
            "timing_state": timing_state,
            "reasons": reasons,
            "extension_level": dist_from_ema,
            "rsi_location": rsi_now,
        }
...
class RegimeTransitionModel:
    """
    Predicts likely NEXT regime based on current conditions.
    More valuable than just knowing current regime.
    """
    def __init__(self):
        self.transition_history = []
        self.transition_probs = {
            ("RANGING", "EXPANSION"): 0.0,
            ("RANGING", "TRENDING"): 0.0,
            ("TRENDING", "EXHAUSTION"): 0.0,
            ("TRENDING", "RANGING"): 0.0,
            ("NORMAL", "TRENDING"): 0.0,
            ("NORMAL", "RANGING"): 0.0,
        }
        self._sample_counts = {k: 1 for k in self.transition_probs}  # Smoothing

    def predict_transition(self, state: dict, df) -> dict:
        """
        Estimate probability of regime transitions.
        """
        current_regime = state.get("regime", "NORMAL")
        trend_score = state.get("trend_score", 0.5)
        range_score = state.get("range_score", 0.5)
        vol_rank = state.get("vol_rank", 0.5)
        bbw_rank = state.get("bbw_rank", 0.5)
        
        # Transition indicators
        squeeze_release = bbw_rank > 0.7 and vol_rank > 0.6
        trend_exhaustion = trend_score > 0.7 and state.get("reversal_prob", 0.5) > 0.4
        range_breakout = range_score > 0.6 and squeeze_release
        
        probs = {}
        
        if current_regime == "RANGING":
            probs["EXPANSION"] = clamp(0.2 + squeeze_release * 0.4, 0.1, 0.7)
            probs["TRENDING"] = clamp(0.15 + range_breakout * 0.35, 0.1, 0.6)
            probs["RANGING"] = clamp(1.0 - probs.get("EXPANSION", 0.3) - probs.get("TRENDING", 0.2), 0.2, 0.8)
        elif current_regime == "TRENDING":
            probs["EXHAUSTION"] = clamp(0.15 + trend_exhaustion * 0.4, 0.1, 0.6)
            probs["RANGING"] = clamp(0.2 + (1.0 - trend_score) * 0.3, 0.1, 0.5)
            probs["TRENDING"] = clamp(1.0 - probs.get("EXHAUSTION", 0.2) - probs.get("RANGING", 0.2), 0.2, 0.8)
        else:  # NORMAL
            probs["TRENDING"] = clamp(0.25 + trend_score * 0.3, 0.15, 0.5)
            probs["RANGING"] = clamp(0.25 + range_score * 0.3, 0.15, 0.5)
            probs["NORMAL"] = clamp(1.0 - probs.get("TRENDING", 0.3) - probs.get("RANGING", 0.3), 0.2, 0.7)
        
        # Find most likely next regime
        most_likely_next = max(probs.keys(), key=lambda k: probs[k]) if probs else current_regime
        transition_confidence = probs.get(most_likely_next, 0.33)
        ...
        return {
            ...
            "is_transition_likely": transition_confidence > 0.45 and most_likely_next != current_regime,
        }

    def record_transition(self, from_regime: str, to_regime: str):
        """Update transition statistics from observed outcomes."""
        key = (from_regime, to_regime)
        if key in self.transition_probs:
            count = self._sample_counts[key]
            current_prob = self.transition_probs[key]
            # Online update with recency
            new_count = count + 1
            alpha = 0.1  # Learning rate
            new_prob = current_prob * (1 - alpha) + alpha * 1.0  # Observed transition
            self.transition_probs[key] = new_prob
            self._sample_counts[key] = new_count

#============================================================================
# SIGNAL TRAJECTORY TRACKER
#============================================================================

class SignalTrajectoryTracker:
    """
    Tracks how signal quality evolves over time.
    Strengthening signals are more reliable than weakening ones.
    """
    def __init__(self, window_size: int = 8):
        self.window_size = window_size
        self.buy_history = deque(maxlen=window_size)
        self.sell_history = deque(maxlen=window_size)
        self.timestamps = deque(maxlen=window_size)

    def update(self, buy_score: float, sell_score: float, ts: float):
        """Record current signal state."""
        self.buy_history.append(buy_score)
        self.sell_history.append(sell_score)
        self.timestamps.append(ts)

    def get_trajectory(self) -> dict:
        """
        Analyze signal trajectory.
        Returns strengthening/weakening/flipping status.
        """
        if len(self.buy_history) < 3:
            return {
        ...
        buy_arr = np.array(self.buy_history)
        sell_arr = np.array(self.sell_history)

        # Calculate trend (slope) using linear regression
        x = np.arange(len(buy_arr))

        if len(buy_arr) >= 3:
            buy_slope = np.polyfit(x, buy_arr, 1)[0]
            sell_slope = np.polyfit(x, sell_arr, 1)[0]
        else:
            buy_slope = 0.0
            sell_slope = 0.0

        # Recent change (last 3 vs previous 3)
        mid = len(buy_arr) // 2
        recent_buy = np.mean(buy_arr[mid:])
        earlier_buy = np.mean(buy_arr[:mid]) if mid > 0 else recent_buy
        buy_change = recent_buy - earlier_buy

        recent_sell = np.mean(sell_arr[mid:])
        earlier_sell = np.mean(sell_arr[:mid]) if mid > 0 else recent_sell
        sell_change = recent_sell - earlier_sell

        # Determine trajectory state
        max_score = max(buy_arr[-1], sell_arr[-1])
        dominant = "BUY" if buy_arr[-1] >= sell_arr[-1] else "SELL"
        ...
        if slope > 0.03 and change > 0.02:
            trajectory = "STRENGTHENING"
        elif slope < -0.03 and change < -0.02:
            trajectory = "WEAKENING"
        elif abs(slope) < 0.02 and abs(change) < 0.02:
            trajectory = "STABLE"
        elif buy_arr[-1] < sell_arr[-1] and buy_arr[-2] >= sell_arr[-2]:
            trajectory = "FLIPPING_TO_SELL"
        elif sell_arr[-1] < buy_arr[-1] and sell_arr[-2] >= buy_arr[-2]:
            trajectory = "FLIPPING_TO_BUY"
        else:
            trajectory = "TRANSITIONAL"

        return {
            "trajectory": trajectory,
            "buy_trend": float(buy_slope),
            "sell_trend": float(sell_slope),
            "strength_change": float(change),
            "recent_buy": float(buy_arr[-1]) if len(buy_arr) > 0 else 0.0,
            "recent_sell": float(sell_arr[-1]) if len(sell_arr) > 0 else 0.0,
            "dominant": dominant,
        }
...
class AdversePathModel:
    """
    Predicts what happens BEFORE the favorable move.
    Critical for managing drawdown expectations.
    """
    def __init__(self):
        self.adverse_stats = {
            "buy": {"count": 0, "avg_adverse": 0.0, "max_adverse": 0.0},
            "sell": {"count": 0, "avg_adverse": 0.0, "max_adverse": 0.0},
        }

    def estimate_adverse_path(self, direction: int, state: dict) -> dict:
        """
        Estimate expected adverse excursion before favorable move.
        """
        atr = max(safe_float(state.get("atr", 1.0)), XAUUSD_POINT)
        vol_rank = state.get("vol_rank", 0.5)

        # Base adverse expectation (higher in high vol)
        base_adverse_r = 0.35 + vol_rank * 0.35

        # Adjust for market regime
        regime = state.get("regime", "NORMAL")
        if regime == "RANGING":
            base_adverse_r *= 1.2  # More whipsaws in ranges
        elif regime == "TRENDING":
            base_adverse_r *= 0.85  # Cleaner moves in trends

        # Direction-specific adjustment
        dir_key = "buy" if direction > 0 else "sell"
        stats = self.adverse_stats.get(dir_key, {"avg_adverse": base_adverse_r})

        # Blend historical with model estimate
        if stats["count"] >= 10:
            adverse_r = 0.6 * base_adverse_r + 0.4 * stats["avg_adverse"]
        else:
            adverse_r = base_adverse_r

        # Probability of significant adverse move (>0.5R)
        prob_significant_adverse = clamp(0.25 + vol_rank * 0.35, 0.15, 0.65)

        # Expected time to recovery (in seconds, rough estimate)
        expected_recovery_time = 15 + vol_rank * 25  # Higher vol = longer recovery

        return {
            "expected_adverse_r": clamp(adverse_r, 0.15, 1.2),
            "prob_significant_adverse": prob_significant_adverse,
            "expected_recovery_time": expected_recovery_time,
            "adverse_scenario": "MODERATE" if adverse_r < 0.5 else "SEVERE",
        }

    def record_outcome(self, direction: str, adverse_r: float):
        """Update adverse path statistics from actual trade outcome."""
        dir_key = direction.lower()
        if dir_key not in self.adverse_stats:
            return

        stats = self.adverse_stats[dir_key]
        count = stats["count"]
        avg = stats["avg_adverse"]
        max_adv = stats["max_adverse"]

        # Online update
        new_count = count + 1
        new_avg = (avg * count + adverse_r) / new_count
        new_max = max(max_adv, adverse_r)

        self.adverse_stats[dir_key] = {
            "count": new_count,
            "avg_adverse": new_avg,
            "max_adverse": new_max,
        }
...
class MetaDecisionLayer:
    """
    Final decision layer: TAKE / WAIT / SKIP
    Considers all predictive inputs before execution.
    """
    def __init__(self):
        pass

    def decide(self, signal: dict, forward_pred: dict, location: dict,
               timing: dict, adverse: dict, model_health: dict,
               trajectory: dict, regime_trans: dict) -> dict:
        """
        Make final TAKE/WAIT/SKIP decision.
        """
        entry_score = signal.get("entry_score", 0.5)
        threshold = signal.get("threshold", 0.54)
        direction = signal.get("direction", "NO_TRADE")

        if direction == "NO_TRADE":
            return {"decision": "SKIP", "reason": "No directional signal"}

        # Extract key metrics
        pred_prob = forward_pred.get("avg_probability", 0.5)
        pred_return = forward_pred.get("avg_expected_return", 0.0)
        pred_aligned = forward_pred.get("aligned_bullish", False) or forward_pred.get("aligned_bearish", False)
        pred_trend = forward_pred.get("trend", 0.0)

        timing_quality = timing.get("timing_quality", 0.5)
        timing_state = timing.get("timing_state", "NEUTRAL")

        location_score = location.get("location_score", 0.5)

        adverse_prob = adverse.get("prob_significant_adverse", 0.3)
        adverse_r = adverse.get("expected_adverse_r", 0.5)

        model_status = model_health.get("overall_status", "OPERATIONAL")
        model_accuracy = model_health.get("avg_accuracy", 0.5)

        traj_state = trajectory.get("trajectory", "STABLE")

        trans_conf = regime_trans.get("transition_confidence", 0.0)
        likely_next = regime_trans.get("most_likely_next", "NORMAL")
        ...
        if pred_trend > 0.02:
            take_score += 0.10
        if timing_quality > 0.60:
            take_score += 0.15
        if location_score > 0.55:
            take_score += 0.10
        if model_status == "OPERATIONAL" and model_accuracy > 0.52:
            take_score += 0.12
        if traj_state == "STRENGTHENING":
            take_score += 0.10

        # Negative factors
        if timing_state in ("POOR", "VERY_POOR"):
            skip_reasons.append("Poor timing")
            take_score -= 0.25
        elif timing_state == "ACCEPTABLE":
            wait_reasons.append("Timing acceptable but not good")
            take_score -= 0.10

        if location_score < 0.35:
            skip_reasons.append("Poor location")
            take_score -= 0.20
        elif location_score < 0.45:
            wait_reasons.append("Suboptimal location")
            take_score -= 0.10

        if adverse_prob > 0.50:
            wait_reasons.append("High adverse path risk")
            take_score -= 0.15

        if model_status == "CRITICAL":
            skip_reasons.append("Model health critical")
            take_score -= 0.40
        elif model_status == "WARNING":
            wait_reasons.append("Model health warning")
            take_score -= 0.20

        if traj_state == "WEAKENING":
            wait_reasons.append("Signal weakening")
            take_score -= 0.15
        elif traj_state == "FLIPPING_TO_SELL" and direction == "BUY":
            skip_reasons.append("Signal flipping against direction")
            take_score -= 0.30
        elif traj_state == "FLIPPING_TO_BUY" and direction == "SELL":
            skip_reasons.append("Signal flipping against direction")
            take_score -= 0.30

        if trans_conf > 0.3 and likely_next != state.get("regime", "NORMAL"):
            wait_reasons.append(f"Regime transition likely to {likely_next}")
            take_score -= 0.12

        # Entry score threshold check
        if entry_score < threshold:
            skip_reasons.append("Below entry threshold")
            take_score -= 0.35
        elif entry_score < threshold + 0.05:
            wait_reasons.append("Marginal entry score")
            take_score -= 0.10

        # Make decision
        if len(skip_reasons) >= 2 or take_score < 0.15:
            decision = "SKIP"
            reason = "; ".join(skip_reasons) if skip_reasons else "Low overall score"
        elif len(wait_reasons) >= 2 or (take_score < 0.45 and timing_state != "EXCELLENT"):
            decision = "WAIT"
            reason = "; ".join(wait_reasons) if wait_reasons else "Awaiting better conditions"
        else:
            decision = "TAKE"
...
# Global state reference for meta-decision (will be updated in main loop)
state = {}
...
def get_dashboard_api():
    return json.dumps(dashboard_data, default=json_default)

def run_dashboard_server():
    log(f"Starting dashboard at http://{DASHBOARD_HOST}:{DASHBOARD_PORT}")
    ...
    for path in chrome_paths:
        if os.path.exists(path):
            try:
                ...
                log("Dashboard opened in Google Chrome")
                return
            except: pass
    webbrowser.open(url)
    log("Dashboard opened in default browser")
